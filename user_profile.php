<?php 
    ...
    while($row = $userbio->fetch_assoc()){
        ...
        echo "<div class='ub-header'>
             <span style = 'color: ".$row["color"]."' class='ub-username'>".$row['username']."</span>
             <i onclick='openchats(1)' style='float:right;color:#303030;cursor:pointer;font-size:40px;margin-right:20px;' 
             class='fas fa-comment-dots' title='Chats'></i>
             </div><br><br><br>";
        echo "<div style='text-shadow:4px 4px 4px ".$row["color"]."' class='ub-name'>".$row['firstname']." ".$row['lastname']."</div><br><br><br>";
        echo "<div class='ub-email'>".$row['email']."</div><br><hr>";
        echo "<p class='ub-line likers'><label class='ub-fields likers'>Likers &nbsp;&nbsp;</label><div class='likelist' disabled ></div></p>";
